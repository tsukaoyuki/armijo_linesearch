...
def check_armijo_conditions(step_size, step_size_old, loss, grad_current,direction_current,
    ...
    found = 0
    g_d=compute_norm(grad_current,direction_current,device)
    break_condition = loss_next - \
        (loss - (step_size) * c * g_d**2)
...
def compute_grad_norm(grad_list,device):
...
def compute_norm(grad_current,direction_current,device):
    g_d_norm=torch.tensor(0.,device=device)
    for g,d in zip(grad_current,direction_current):
        if g is None:
            continue
        g_t=torch.flatten(g)
        d_t=torch.flatten(d)
        g_d_norm+=torch.sum(torch.dot(g_t,d_t))
    g_d_norm=torch.sqrt(g_d_norm)
    return g_d_norm
